...
class SistemaCine:
    ...
    def agregar_relacion(self, id_estrella, id_pelicula, personaje):
        if self.usuario_actual:
            ...
            relacion = Relaciones(new_id, id_estrella, id_pelicula, personaje)
            self.relaciones[relacion.id_relacion] = relacion
...
if __name__ == '__main__':
    ...
    for id_estrella, actor in actores.items():
        print(f"{id_estrella}: {actor.nombre:30s} - {actor.fecha_nacimiento}")
    ...
    u = sistema.usuarios['adrisanv']
    print(u.username)
    print(u.password)
    print(u.hash_string(u.password))
    exito = sistema.login('adrisanv','12345')
    print(exito)
   
    if (exito):
        print(sistema.usuario_actual.username)
        sistema.agregar_pelicula('la vida es bella','1997-12-20','https://m.media-amazon.com/images/I/71JtXzFgQhL._AC_UF894,1000_QL80_.jpg')
